...
It turns out there’s a wide variety of chat templates across the LLM research community. Take an open-source model like Mixtral-8x7B-Instruct-v0.1. Its format looks wildly different from gpt-3.5-turbo above:
...
Why bother with chat templates? Well, it’s strongly advised to match the expected chat template at prediction time (for instance, see the info on "Instruction format" at the [repo](https://huggingface.co/mistralai/Mixtral-8x7B-Instruct-v0.1) for Mixtral-8x7B-Instruct-v0.1). And, with proprietary chat models like gpt-3.5-turbo, chat templates are often applied behind the scenes of an endpoint whether you like it or not!
...
| Model | Without Chat Template | With Chat Template | Difference |
|---|---|---|---|
| Llama-2-70b-chat-hf | ~54% | ~56% | +2% |
| Mixtral-8x7B-Instruct-v0.1 | ~59% | ~60% | +1% |
| Nous-Hermes-2-Mixtral-8x7B-DPO | ~63% | ~63% | +0% |
| Nous-Hermes-2-Yi-34B | ~48% | ~51% | +3% |
| Starling-LM-7B-alpha | ~50% | ~52% | +2% |
| Zephyr-7b-beta | ~24% | ~63% | +39% |
| Mistral-7B-Instruct-v0.2 | ~45% | ~49% | +4% |
